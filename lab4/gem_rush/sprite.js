...
const Sprite = function (ctx, x, y) {
    ...
    const update = function (time) {
        ...
        const elapsedTime = time - lastUpdate;

        if (elapsedTime >= sequence.timing) {
            const framesToAdvance = Math.floor(elapsedTime / sequence.timing);
            index += framesToAdvance;
            if (sequence.loop) {
                index %= sequence.count;
            } else {
                index = Math.min(index, sequence.count - 1);
            }
            lastUpdate += framesToAdvance * sequence.timing;
        }
        return this;
    };
